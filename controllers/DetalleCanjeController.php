...
class detallecanje
{
    ...
    public function getForm($id){   
        ...
        if(isset($id) && !empty($id) && $id!=='undefined' && $id!=='null'){
            //Acción del modelo a ejecutar
            $response=$DetalleCanje->getForm($id);
            //Verificar respuesta
            if(isset($response) && !empty($response)){
                //Armar el json
                $json=array(
    ...
    public function getDetalleByEncabezado($id){
        ...
        $DetalleCanje=new DetalleCanjeModel();
        //Acción del modelo a ejecutar
        $response=$DetalleCanje->getDetalleByEncabezado($id);
        //Verificar respuesta
        if(isset($response) && !empty($response)){
            //Armar el json
            $json=array(
    ...
    public function create()
    {
        ...
        $DetalleCanje = new DetalleCanjeModel();
        //Acción del modelo a ejecutar
        $response = $DetalleCanje->create($object);
        //Verificar respuesta
        if (isset($response) && !empty($response)) {
            $json = array(
                'status' => 200,
    ...
    public function update()
    {
        ...
        $DetalleCanje = new DetalleCanjeModel();
        //Acción del modelo a ejecutar
        $response = $DetalleCanje->update($object);
        //Verificar respuesta
        if (isset($response) && !empty($response)) {
            $json = array(
                'status' => 200,
                'results' => "detalle actualizada"
            );
        } else {
            $json = array(
                'status' => 400,
